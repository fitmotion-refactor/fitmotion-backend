...
class ChatServiceImplTest {
    ...
    void 메시지를_저장하면_DTO로_반환된다() {
        ...
        ChatMessage savedEntity = ChatMessage.builder()
                .messageId(100L).senderId(1L).receiverId(2L).senderRole("USER").content("안녕하세요")
    ...
    void 두_사용자간의_채팅기록을_조회한다() {
        ...
        ChatMessage msg1 = ChatMessage.builder()
                .messageId(1L).senderId(1L).receiverId(2L).content("첫 메시지").senderRole("USER")
                .timestamp(LocalDateTime.now()).build();

        ChatMessage msg2 = ChatMessage.builder()
                .messageId(2L).senderId(2L).receiverId(1L).content("답장").senderRole("TRAINER")
                .timestamp(LocalDateTime.now()).build();
